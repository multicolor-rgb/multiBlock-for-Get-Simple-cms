	...
	<?php 
		...
		if(isset($_GET['newmulticategory']) || isset($_GET['refresh']) ){
			...
			foreach (glob(GSDATAOTHERPATH."/oneBlock/".str_replace(" ","-",$_GET['newmulticategory'])."/*.json") as $filename) {
				...
				<a href="'.$SITEURL.'admin/load.php?id=multiBlock&newblock1&newmulticategory='.$_GET['newmulticategory'].'&namefile='.$name.'" class="mb_edit" style="width:45px;">'.i18n_r("multiBlock/EDIT").'</a>
...
<script>
	document.querySelector('.cats').setAttribute('action','<?php global $SITEURL;echo $SITEURL;?>admin/load.php?id=multiBlock&newoneblock&newmulticategory='+document.querySelector('.mb_category').value);

	if(document.querySelector('.addnewblock')!==null){
		document.querySelector('.addnewblock').setAttribute('href','<?php global $SITEURL;echo $SITEURL;?>admin/load.php?id=multiBlock&newblock1&newmulticategory='+document.querySelector('.mb_category').value);
	}
