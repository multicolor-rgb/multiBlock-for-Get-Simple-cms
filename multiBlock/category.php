	...
	foreach (glob(GSDATAOTHERPATH . "/multiBlock/category/*.json") as $filename) {
		...
		echo '<li>
			<p>' . $content[0]['name'] . ' <span style="opacity:0.5;font-size:12px;margin-left:10px">slug:' . basename($filename, '.' . $info['extension']) . '</span></p>
			 <div><a href="' . $SITEURL . 'admin/load.php?id=multiBlock&addnew&&categorytitle=' . $content[0]['name'] . '&categoryname=' . basename($filename, '.' . $info['extension']) . '"
...
<ul class="mb_catlist">
	...
	foreach (glob(GSDATAOTHERPATH . "/oneBlock/category/*.json") as $filename) {
		...
		echo '<li><p>' . $content[0]['name'] . '<span style="opacity:0.5;font-size:12px;margin-left:10px">slug:' . basename($filename, '.' . $info['extension']) . '</span></p> <div>
		<a href="' . $SITEURL . 'admin/load.php?id=multiBlock&addnewOneBlock&categorytitle=' . $content[0]['name'] . '&categoryname=' . basename($filename, '.' . $info['extension']) . '"
